...
def plot_confusion_matrix(cm, classes, set_name="", normalize=False, save_path=None, cmap=plt.cm.Blues):
    ...
    if normalize:
        cbar.set_label('Normalized Frequency', fontname='Arial', fontsize=22)
    else:
        cbar.set_label('Count', fontname='Arial', fontsize=22)

    # 设置坐标轴标签和刻度（使用排序后的类别）
    tick_marks = np.arange(len(sorted_classes))
    plt.xticks(tick_marks, sorted_classes, rotation=45, fontname='Arial', fontsize=20)
    plt.yticks(tick_marks, sorted_classes, fontname='Arial', fontsize=20)
...
plt.xticks(n_estimators_list[::10], fontname='Arial', fontsize=20)
...
plt.tight_layout()
plt.savefig('accuracy_plot_macro.png', dpi=600, bbox_inches='tight')
plt.close()
...
try:
    ...
    plt.yticks(np.arange(len(sorted_idx)), [feature_names[i] for i in sorted_idx],
    ...
    ax.invert_yaxis()
    plt.tight_layout()
    plt.savefig(f"{shap_dir}/global_feature_importance_rank.png", dpi=600, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{shap_dir}/feature_effects_distribution.png", dpi=600)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{shap_dir}/top20_feature_heatmap.png", dpi=600)
    plt.close()
...
feature_names = X.columns.tolist()
n_classes = len(Y.unique())  # 自动获取类别数（此处应为4）
class_names = sorted(Y.unique().tolist())  # 修改：确保类别按数字顺序排序
...
def plot_decision_curve(y_true, y_proba, classes, set_name="", save_path=None, colors=None,
    ...
    for boot_idx in range(n_bootstraps):
        ...
        y_proba_boot = y_proba_sorted[indices]
    ...
    for class_idx in range(n_classes):
        ...
        # 绘制平均净收益曲线（加粗至5，统一颜色）
        curve, = plt.plot(thresholds, class_mean_benefit, lw=5, color=color)
        curve_objects.append(curve)
        # 修改：固定使用Class 0, Class 1, Class 2, Class 3
        curve_labels.append(f'Class {class_idx}')
    ...
    # 计算参考线（Treat None/Treat All）
    overall_prevalence = np.mean([(y_true == i).mean() for i in range(n_classes)])
    treat_all_benefit = overall_prevalence - (1 - overall_prevalence) * thresholds

    # 绘制参考线
    none_line = plt.axhline(y=0, color='black', linestyle='--', lw=5)
    all_line = plt.plot(thresholds, treat_all_benefit, color='navy', linestyle=':',
                        alpha=0.7, lw=5)[0]
    ...
    plt.title(f'Decision Curve ({set_name})',
              fontname='Arial', fontsize=25)

    # 固定顺序创建图例：Class 0, Class 1, Class 2, Class 3, Treat none, Treat all
    sorted_objects = curve_objects.copy()
    sorted_labels = curve_labels.copy()

    # 添加参考线到图例
    sorted_objects.extend([none_line, all_line])
    sorted_labels.extend(['Treat none', 'Treat all'])

    plt.legend(sorted_objects, sorted_labels,
               loc='lower left',
    ...
    # 保存交点结果到Excel
    if intersection_data and save_path:
        ...
        excel_path = os.path.join(save_dir, f"{set_name.lower()}_decision_curve_intersections_with_xy.xlsx")
        intersection_df.to_excel(excel_path, index=False)
        print(f"{set_name}决策曲线交点结果已保存至: {excel_path}")
...
def plot_roc_curves(y_true_binarized, y_proba, classes, set_name="", save_path=None, colors=None):
    ...
    # 绘制每个类别的ROC曲线：宽度设为5，确保按0,1,2,3的顺序
    for class_idx in range(n_classes):
        fpr, tpr, _ = roc_curve(y_true_binarized[:, class_idx], y_proba_sorted[:, class_idx])
        roc_auc = auc(fpr, tpr)
        color = colors[class_idx % len(colors)]
        curve, = plt.plot(fpr, tpr, color=color, lw=5)
        curve_objects.append(curve)
        # 修改：固定使用Class 0, Class 1, Class 2, Class 3
        curve_labels.append(f'Class {class_idx} (AUC = {roc_auc:.4f})')

    # 对角线参考线
    diagonal_line, = plt.plot([0, 1], [0, 1], 'k--', lw=5)
    ...
    # 添加对角线到图例
    sorted_objects.append(diagonal_line)
    sorted_labels.append('Chance')
...
def plot_pr_curves(y_true_binarized, y_proba, classes, set_name="", save_path=None, colors=None):
    ...
    # 绘制每个类别的PR曲线：宽度设为5，确保按0,1,2,3的顺序
    for class_idx in range(n_classes):
        precision, recall, _ = precision_recall_curve(y_true_binarized[:, class_idx], y_proba_sorted[:, class_idx])
        ap = np.trapz(precision, recall)  # 计算AP
        color = colors[class_idx % len(colors)]
        curve, = plt.plot(recall, precision, color=color, lw=5)
        curve_objects.append(curve)
        # 修改：固定使用Class 0, Class 1, Class 2, Class 3
        curve_labels.append(f'Class {class_idx} (AP = {ap:.4f})')
...
# ==================== 调用绘图函数 ====================
print("\n=== Plotting Decision Curves, ROC and PR Curves ===")

# 定义颜色方案
plot_colors = ['#88C4D7', '#D0EAD5', '#AFADD2', '#E6C7DF']

# 创建保存目录
for dir_name in ["decision_curves", "roc_curves", "pr_curves"]:
    os.makedirs(dir_name, exist_ok=True)

# 为每个数据集绘制所有曲线
datasets_info = [
    ('Training', Y_Train, y_train_proba, y_train_binarized),
    ('Validation', Y_Val, y_val_proba, y_val_binarized),
    ('Test', Y_Test, y_test_proba, y_test_binarized)
]

for set_name, y_true, y_proba, y_binarized in datasets_info:
    print(f"\nProcessing {set_name} set...")

    # 1. 绘制决策曲线
    dc_save_path = f"decision_curves/{set_name.lower()}_decision_curve.png"
    plot_decision_curve(
        y_true, y_proba,
        classes=class_names,
        set_name=set_name,
    ...
    print(f"  Decision curve saved: {dc_save_path}")

    # 2. 绘制ROC曲线
    roc_save_path = f"roc_curves/{set_name.lower()}_roc_curve.png"
    plot_roc_curves(
        y_binarized, y_proba,
        classes=class_names,
        set_name=set_name,
    ...
    # 3. 绘制PR曲线
    pr_save_path = f"pr_curves/{set_name.lower()}_pr_curve.png"
    plot_pr_curves(
        y_binarized, y_proba,
        classes=class_names,
        set_name=set_name,
    ...
    print(f"  PR curve saved: {pr_save_path}")

print("\nAll curves have been plotted successfully!")
print("图例顺序已固定为: Class 0, Class 1, Class 2, Class 3")
