...
def to_one_hot_3d(tensor, n_classes, device='cuda'):
    n, s, h, w = tensor.shape
    # Create one-hot tensor on the specified device
    one_hot = torch.zeros(n, n_classes, s, h, w, device=device)
    # Scatter needs the index tensor to be on the same device
    one_hot.scatter_(1, tensor.view(n, 1, s, h, w).to(device), 1)
    return one_hot
