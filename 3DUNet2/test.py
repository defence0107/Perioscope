...
# 模型测试主函数（优化：分块推理+内存清理）
def test_model(model, test_dataset, output_dir='test_results', visualize=True, save_itk=True, num_classes=2,
               use_half=False):
    """分块推理+内存清理：大幅降低峰值内存"""
    ...
    with torch.no_grad():
        for data_loader, img_name in tqdm(test_dataset, desc="测试进度"):
            print(f"\n处理图像: {img_name}")
...
if __name__ == "__main__":
    ...
    # 模型路径
    model_path = os.path.join('saved_models', 'best_model_weights.pth')  # 加载最佳模型权重

    # 测试数据路径
    test_data_path = args.test_data_path  # 从配置中获取测试数据路径
    ...
    # 2. 加载测试数据
    print("准备测试数据集...")
    from dataset_lits_test import Test_Datasets  # 根据实际情况调整导入路径

    test_dataset = Test_Datasets(test_data_path, args)
