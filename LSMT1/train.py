...
from model import LSTM  # 假设模型已支持四分类输出
import pandas as pd
...
class LSTMTrainer:
    def __init__(self, model, train_loader, val_loader=None, test_loader=None,
        ...
        # 损失函数和优化器（四分类使用CrossEntropyLoss）
        self.criterion = nn.CrossEntropyLoss()
        self.optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)

        # 训练历史记录（四分类评估指标，修改num_classes为4）
        self.history = {
            'train_loss': [],
            'train_acc': [],
            'train_auc': [],
    ...
    def _calculate_metrics(self, outputs, labels):
        ...
        # 计算AUC（四分类使用ovr方式，修改num_classes为4）
        try:
            if outputs.shape[1] == 4:  # 四分类情况
                # 将标签转为one-hot编码
    ...
    def generate_boxplot_data(self, save_dir='./prob_distribution_data', bins=30):
        """生成用于特征概率分布的数据并保存到Excel"""
        print("\n开始生成特征概率分布数据...")
        self.model.eval()
        device = self.device
        ...
        if self.train_loader:
            with torch.no_grad():
                for inputs, _ in tqdm(self.train_loader, desc="收集训练集特征"):
        ...
        if self.val_loader:
            with torch.no_grad():
                for inputs, _ in tqdm(self.val_loader, desc="收集验证集特征"):
                    inputs = inputs.to(device)
                    all_features.append(inputs.cpu().numpy())
        ...
        if self.test_loader:
            with torch.no_grad():
                for inputs, _ in tqdm(self.test_loader, desc="收集测试集特征"):
                    inputs = inputs.to(device)
                    all_features.append(inputs.cpu().numpy())
        ...
        # 特征名称（需与数据集中的特征顺序一致）
        feature_names = ["CAL", "Periodontal pocket", "Jawbone loss", " loosenes", "age",
                         "weight", "Smoke", "Educational level",
                         "trouble sleeping", "Smoking frequency", "Degree of smoking",
                         "healthy diet", "trouble sleep", "gender", "Drink", "Drinking frequency",
                         "Annual family income"]

        # 确保特征名称数量与特征维度一致
        if all_features.shape[1] != len(feature_names):
            feature_names = [f"feature_{i}" for i in range(all_features.shape[1])]
            print(f"警告：未找到特征名称，使用默认名称 {feature_names}")
        ...
        output_file = os.path.join(save_dir, "feature_prob_distribution_data.xlsx")
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            for feature in feature_names:
                ...
                # 计算概率分布
                hist, bin_edges = np.histogram(feature_data, bins=bins, density=True)
                ...
                # 保存到Excel
                dist_df.to_excel(writer, sheet_name=feature, index=False)

            print(f"特征概率分布数据已保存至：{output_file}")
            print("数据格式说明：")
            print(" - 每个特征对应一个工作表，包含'bin_start'、'bin_end'和'probability_density'三列")

        return feature_df
    def train(self, epochs, save_dir='./saved_models'):
        """完整训练流程（保持原有逻辑，修改num_classes相关部分）"""
        ...
        # 训练完成后生成箱型图数据
        print("\n训练完成，开始生成箱型图数据...")
        self.generate_boxplot_data(save_dir)
    ...
    def plot_confusion_matrix(self, save_dir, dataset_type='train'):
        ...
        num_classes = cm.shape[0]  # 四分类时为4

        plt.figure(figsize=(12, 10))  # 增大图形尺寸以适应四分类
    ...
    def plot_calibration_curve(self, save_dir, dataset_types=['test']):
        ...
        for dataset_type in dataset_types:
            ...
            all_probs = np.array(all_probs)
            all_labels = np.array(all_labels)
            num_classes = all_probs.shape[1]  # 四分类时为4

            plt.figure(figsize=(12, 10))  # 增大图形尺寸
            ax1 = plt.subplot(111)
            ax1.plot([0, 1], [0, 1], "k:", label="Perfectly calibrated")

            colors = ['#B7DBE3', '#BCAFFF', '#A7AEE2', '#E6C7DF']
            for i in range(num_classes):
    ...
    def _find_crossings(self, x, y1, y2):
        """
        辅助函数：寻找两条曲线y1和y2在x范围内的所有交点
        返回交点的(x, y)坐标列表
        """
        crossings = []
        # 计算差值的符号变化
        ...
        for idx in sign_changes:
            ...
            # 获取线段端点
            x0, x1 = x[idx], x[idx + 1]
            y0_1, y1_1 = y1[idx], y1[idx + 1]
            y0_2, y1_2 = y2[idx], y2[idx + 1]
            ...
            y_intersect = y0_1 + t * (y1_1 - y0_1)

            # 确保交点在当前区间内
            if min(x0, x1) <= x_intersect <= max(x0, x1):
...
if __name__ == "__main__":
    ...
    # 2. 初始化LSTM模型（四分类，修改num_classes为4）
